...
| Finnhub (`finnhub.io/api/v1/calendar/economic`) | Economic calendar | Free API key(s) (`Finnhub:ApiKey` / `Finnhub:ApiKeys`) | Adapter built (`Providers/FinnhubEconomicCalendarProvider.cs`), not yet wired into any endpoint. Returns `Available:false` with a reason until a key is configured. |
